...
# ============================================================
# SADDLE POINT FUNCTION (Hyperbolic Paraboloid)
# f(x, y) = x² - y²
...
# For Basic Newton's, let's find roots of g(x,y) = [x² - 1, y² - 4]
def g_vector(x, y):
...
def newtons_optimization(x0, y0, max_iter=20, tol=1e-6, verbose=True):
    ...
    if verbose:
        ...
        print("="*60)
        print(f"Goal: Find critical point of f(x,y) = x² - y²")
        print(f"Note: (0,0) is a SADDLE POINT, not a minimum!")
        print(f"Starting point: ({x0:.3f}, {y0:.3f})")
        print("-"*60)
...
def plot_results(history_basic, history_newton, history_bfgs, history_cg, history_gd, save_dir='.'):
    ...
    for idx, (title, history) in enumerate(zip(titles, histories)):
        ...
        if idx > 0:
            ax.scatter([0], [0], [0], color='gold', s=200, marker='*', 
                      label='Saddle Point', edgecolors='black', zorder=10)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/saddle_bfgs_3d.png', dpi=150, bbox_inches='tight')
    print(f"\n3D plot saved to: {save_dir}/saddle_bfgs_3d.png")
...
def plot_contours(history_basic, history_newton, history_bfgs, history_cg, history_gd, save_dir='.'):
    ...
    for ax, title, history in zip(axes[:5], titles, histories):
        ...
        contour = ax.contour(X, Y, Z, levels=levels, cmap='coolwarm', alpha=0.6)
        ax.clabel(contour, inline=True, fontsize=8)

        ax.scatter([0], [0], color='gold', s=200, marker='*', 
                  zorder=7, label='Saddle Point', edgecolors='black')
        ...
        if history:
            ...
            ax.scatter(xs, ys, c='green', s=30, zorder=5)
            ax.scatter([xs[0]], [ys[0]], color='blue', s=100, 
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/saddle_bfgs_contours.png', dpi=150, bbox_inches='tight')
    print(f"Contour plot saved to: {save_dir}/saddle_bfgs_contours.png\n")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("SADDLE POINT FUNCTION OPTIMIZATION")
    print("f(x, y) = x² - y²  (Hyperbolic Paraboloid)")
    print("="*60)
    ...
    print(f"Saddle Point:          (0.000000, 0.000000), f = 0.000000")
    print("="*60)

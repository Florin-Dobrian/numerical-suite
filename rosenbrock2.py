...
def hessian(x, y):
    ...
    H = np.array([[2 + 1200*x**2 - 400*y, -400*x],
                  [-400*x, 200]])
    return H
...
def g_vector(x, y):
    ...
    return np.array([(1 - x) - 20*(y - x**2), y - x**2])

def jacobian(x, y):
    """Jacobian of g"""
    J = np.array([[-1 + 40*x, -20],
                  [-2*x, 1]])
    return J
...
def newtons_optimization(x0, y0, max_iter=20, tol=1e-6, verbose=True):
    ...
    if verbose:
        ...
        print("="*60)
        print(f"Goal: Find minimum of f(x,y) = (1-x)² + 100(y-x²)²")
...
# 4. GRADIENT DESCENT
# ============================================================
def gradient_descent(x0, y0, alpha=0.001, max_iter=10000, tol=1e-6, verbose=True):
    ...
    for i in range(max_iter):
        ...
        if verbose and (i % 500 == 0 or i < 10):
            print(f"Iteration {i}:")
            print(f"  (x, y) = ({x:.6f}, {y:.6f})")
            print(f"  f(x, y) = {f_val:.6f}")
            print(f"  ∇f = [{grad[0]:.6f}, {grad[1]:.6f}]")
...
def plot_results(history_basic, history_newton, history_bfgs, history_cg, history_gd, save_dir='.'):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/rosenbrock_bfgs_3d.png', dpi=150, bbox_inches='tight')
    print(f"\n3D plot saved to: {save_dir}/rosenbrock_bfgs_3d.png")
...
def plot_contours(history_basic, history_newton, history_bfgs, history_cg, history_gd, save_dir='.'):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/rosenbrock_bfgs_contours.png', dpi=150, bbox_inches='tight')
    print(f"Contour plot saved to: {save_dir}/rosenbrock_bfgs_contours.png\n")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("\nThis is the Rosenbrock banana function:")
    ...
    print("\n")
    history_bfgs = bfgs_method(x0, y0, max_iter=1000)
    print("\n")
    history_cg = conjugate_gradient(x0, y0, max_iter=20)
    print("\n")
    history_gd = gradient_descent(x0, y0, alpha=0.001, max_iter=10000)
    ...
    print(f"True Minimum:          (1.000000, 1.000000), f = 0.000000")
    print("="*60)

    print("\nINTERPRETATION:")
    print("For the challenging Rosenbrock function:")
    ...
    print("- Gradient descent: Very slow in the curved valley")
    print("="*60)
